...
export default function Home() {
  ...
  const handleMenuClick = (item: string) => {
    if (item === "Log Out") {
      signOut({ callbackUrl: `${process.env.NEXT_PUBLIC_USER_URL}` });
    } else if (item === "Log In") {
      signIn();
    } else if (item === "ADMIN") {
    ...
    <div>
      <nav className="flex flex-wrap items-center justify-between p-3 bg-[#eeeee9]">
        ...
        <div className="hidden sm:flex items-center justify-end mx-2">
          ...
          {session ? (
            <Button variant="contained" className="px-4 py-2 bg-[#de8cde] text-white" onClick={() => { signOut({ callbackUrl: `${process.env.NEXT_PUBLIC_USER_URL}` }) }}>Sign-Out</Button>
          ) : (
            <Button variant="contained" className="px-4 py-2 mx-2 bg-[#de8cde] text-white" onClick={() => { signIn() }}>Sign-In</Button>
          )}
          <Button variant="contained" className="px-4 py-2 bg-[#de8cde] text-white" onClick={() => { router.push("/sign-up") }}>Sign-Up</Button>
        </div>
